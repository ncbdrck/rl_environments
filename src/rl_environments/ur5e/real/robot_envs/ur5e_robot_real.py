...
register(
    ...
    entry_point='rl_environments.ur5e.real.robot_envs.ur5e_robot_real:UR5eRobotEnv',
    max_episode_steps=1000,
)


class UR5eRobotEnv(RealBaseEnv.RealBaseEnv):
    """
    ...
    def __init__(self, ros_port: str = None, seed: int = None, close_env_prompt: bool = False, action_cycle_time=0.0,
        ...
        Actuators Topic List:
            ...
            /ur5e/arm_controller/command: Send the joint positions to the robot.
            /ur5e/gripper_controller/command: Send the joint positions to the robot
        """
        rospy.loginfo("Start Init UR5eRobotEnv RealROS")
        ...
        # Real UR5e bring-up: hardware driver + Robotiq gripper driver +
        # MoveIt config, all under the /ur5e namespace so the sim and
        # real envs talk to the same topic / service / action paths.
        #
        # IMPORTANT (to verify at the lab): the wrapper launch
        # ur5e_description_extras/launch/ur5e_real.launch is expected
        # to (1) launch ur_robot_driver/launch/ur5e_bringup.launch with
        # the calibrated UR5e IP, (2) launch a Robotiq 2F-85 gripper
        # driver, and (3) include ur5e_move_group.launch. Until that
        # wrapper exists, point robot_launch_file at whatever real
        # bring-up you use; this real env only requires that
        # /ur5e/joint_states + /ur5e/arm_controller + /ur5e/move_group
        # are alive once it returns.
        load_robot = True
        robot_pkg_name = "ur5e_description_extras"
        robot_launch_file = "ur5e_real.launch"
        robot_args = []
        ...
        # MoveIt — planning groups match the upstream
        # ur5e_robotiq_85_moveit_config SRDF: "arm" + "gripper".
        self.move_UR5E_object = MoveitRealROS(arm_name='arm',
        ...
        if self.use_zed2:
            ...
            # rgb image subscriber
            self.zed2_rgb_sub = rospy.Subscriber("/ur5e/zed2/left/image_rect_color", Image,
        ...
        # Robotiq 2F-85: single knuckle (mimic linkage drives the other
        # finger joints).
        ...
        # UR5e URDF uses bare link names (no ur5e/ prefix unlike
        # Interbotix). MoveIt SRDF group "arm" ends at "ee_link".
        ...
        # with ros_kinematics
        self.ros_kin = ros_kinematics.Kinematics_pyrobot(robot_description_parm="ur5e/robot_description",
    ...
    # action to be safe. UR5e URDF uses bare link names. Robotiq links
    # downstream of ee_link via fixed joints — covered implicitly by
    # checking ee_link.
    SAFETY_CHECK_LINKS = (
        "shoulder_link",
        "upper_arm_link",
    ...
    def _check_action_links_safe(self, joint_targets, current_joints=None):
        ...
        """
        # UR5e safety geometry: arm mounted at z=base_z on a separate
        # base; cafe-table top sits at z=table_top_z in front of the arm.
        # The check enforces a base floor for every link plus a stricter
        # table floor only when a link's XY lands over the cafe-table.
        # Same approach as UR5eRobotEnv (sim).
        strict = bool(getattr(self, "enable_strict_safety", False))
        base_z = float(rospy.get_param("/ur5e/base_z", 0.59))
        ...
        # FK returns base_link-relative pose; world z = fk_z + base_z.
        per_link_z = []
        for link, (kin, n) in self._safety_kin.items():
            ...
            over_table_xy = (table_x_min <= x_world <= table_x_max
                             and table_y_min <= y_world <= table_y_max)
        ...
        if self._safety_log_count < 3:
            ...
            rospy.loginfo(
                f"[SAFETY-REAL] call #{self._safety_log_count}: "
                f"base_floor={base_floor:.3f}, table_floor={table_floor:.3f}, {zs}"
            )
    ...
    def _check_zed2_ready(self):
        ...
        """
        rospy.logdebug(rostopic.get_topic_type("/ur5e/zed2/left/image_rect_color", blocking=True))
