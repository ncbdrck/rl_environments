...
"""
NED2 Push Goal Task Environment (real).

Ported from rx200_push_goal_real.py with the NED2-specific conventions taken
from ned2_push_real.py (the just-landed std real push variant) and
ned2_push_goal_sim.py (the goal-env conventions):
  * 6 arm joints (vs 5 on the RX200) — init_pos, smoothing, close-race
    guard (len(jv) < 6), and the comments all reflect this.
  ...
  * No gripper init — NED2 push doesn't open/close (matches sim push +
    reach real). The cube is placed by hand and pushed with the wrist.
...
Goal-env specifics (carried from ned2_push_goal_sim + rx200_push_goal_real):
  * GoalEnv API: compute_reward / compute_terminated / compute_truncated
    (no underscore) — HER plumbing lives in compute_reward and the dense
    path branches on ``is_her`` to drop step-side-effect penalties on
    relabeled transitions.
  * ``_get_achieved_goal`` returns the current cube pos, ``_get_desired_goal``
    returns the sampled push target — both 3-D.
  * ``cube_pos`` / ``cube_ori`` are seeded in ``_set_init_params`` from
    ``cube_init_pos`` so the first ``reset()`` returns sensible values
    even before the first sample_observation tick has fired.

Key sim/real divergences for push-goal specifically:
  * Sim reads cube pose from Gazebo via ``get_model_pose("red_cube")``;
    ...
    keeps the env constructable in a dry-run without vision wired up.
  ...
  * Sim has Gazebo's guaranteed /clock so it doesn't need the joint-
    ...
    ned2_reach_real introduced so we stop the arm if the driver/cable
    drops.
"""


class NED2PushGoalEnv(ned2_robot_goal_real.NED2RobotGoalEnv):
    """
    This Task env is for a simple push Task with the NED2 robot (goal-conditioned).
    ...
    Here
        ...
        * Observation - Dict({'observation', 'achieved_goal', 'desired_goal'})
          where ``observation`` is the flat array (normal_obs / rgb_obs)
          or a nested sensor Dict when multiple sensors are active.
    ...
    def __init__(self, new_roscore: bool = False, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "NED2PushGoalEnvReal" + "_" + ros_port
        else:
            self.node_name = "NED2PushGoalEnvReal"
        ...
        # tasks. NED2RobotGoalEnv (real) keeps ``use_kinect`` / ``use_zed2``
        # (only the sim parent renamed to ``use_camera``) — keep the local
        # variable name aligned with that kwarg.
        if self.normal_obs:
            use_kinect = False
        ...
        # only the sim parent renamed to ``use_camera``. Mirroring
        # ned2_push_real here so the head-mounted kinect2 / ZED2 stream
        ...
        # Seeded with cube_init_pos in _set_init_params so the first
        # reset() returns sensible (not None) values.
        self.cube_pos = None
        self.cube_ori = None
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        Here we
            ...
            2. Prompt the user to physically place the cube (real robot;
               there's no Gazebo spawn/remove on hardware)
            3. Find random goal position to push the cube to if random_goal is True
            4. Publish the goal position and cube position as markers
            5. Seed cube_pos / cube_ori from cube_init_pos so the first
               _get_achieved_goal returns a real number even before the
               vision pipeline has published anything.
        ...
        # No gripper init on NED2 push — matches ned2_push_sim + reach real.
        ...
        # for the initial marker / fallback / cache seed.
        rospy.loginfo(
        ...
        # Seed the cube pose cache from cube_init_pos so _get_achieved_goal
        # returns a real number before the first sample_observation tick has
        # fired. The real env may not have a /cube_pose message yet (vision
        # warmup, mocap not streaming, etc.) — the cached value here will
        # be overwritten the moment sample_observation reads a fresh pose.
        self.cube_pos = cube_init_vector.copy()
    ...
    def sample_observation(self):
        ...
        if self.prev_action is None:
            ...
            # we can use the joint values as the previous action - for a Joint action type
            else:
    ...
    def _get_desired_goal(self):
        """Desired goal = sampled push target for this episode."""
        return np.asarray(self.push_goal, dtype=np.float32).copy()
    ...
    def calculate_reward(self, achieved_goal=None, desired_goal=None, info=None) -> float:
        ...
        Falls back to live env state (self.cube_pos, self.push_goal) for
    ...
    def check_if_done(self):
        ...
        done_reach = self.check_if_reach_done(self.cube_pos, self.push_goal)
    ...
    def _launch_roscore(port=None, default_port=False, set_new_master_vars=False):
        ...
        Mirrors ned2_push_real's static helper — the real-side variant
