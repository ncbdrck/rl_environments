...
# Custom robot env
from rl_environments.ur5e.sim.robot_envs import ur5e_robot_sim
...
class UR5ePnPEnv(ur5e_robot_sim.UR5eRobotEnv):
    """
    ...
    def __init__(self, launch_gazebo: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if launch_gazebo:
            ...
            ros_port, gazebo_port, gazebo_pid = self._launch_gazebo(
                launch_roscore=new_roscore, port=roscore_port,
                paused=gazebo_paused, gui=gazebo_gui,
                custom_world_pkg="ur5e_description_extras",
                custom_world_name="ur5e_scene.world")
        ...
        if ros_port is not None:
            self.node_name = "UR5ePnPEnv" + "_" + ros_port
        else:
            self.node_name = "UR5ePnPEnv"
        ...
        # position; the other finger joints follow via the URDF mimic
        # linkage). Both EE-action and joint-action modes get the extra
        # DOF. Knuckle range: gripper_min (~0.0 rad, fully open) to
        # gripper_max (~0.8 rad, fully closed).
        ...
        # UR5eRobotEnv maps real_time → unpause_pause_physics; this single
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        # --------------- Initial robot pose - folded upright.
        # Matches the reach + push envs. UR5e all-zeros puts the arm
        # horizontal through the cafe-table column, so we start folded
        # with the EE over the workspace.
        self.init_pos = np.array([0.0, -1.5707, 1.5707, -1.5707, -1.5707, 0.0],
                                 dtype=np.float32)

        # Robotiq 2F-85 init poses: open at gripper_open_value (~0.0),
        # closed at gripper_closed_value (~0.8). PnP resets the gripper
        # open so the agent has to command close to grasp.
        self.init_open_gripper = np.array([self.gripper_open_value], dtype=np.float32)
        self.init_close_gripper = np.array([self.gripper_closed_value], dtype=np.float32)
        ...
        # Bypass MoveIt for init (URDF zero pose is in collision with the
        # cafe-table column, so MoveIt's plan-from-collision fails before
        # it tries). Snap joints via SetModelConfiguration (Gazebo
        # paused), then drive both arm and gripper through the low-level
        # trajectory controllers. PnP resets with the gripper OPEN.
        self.move_UR5E_object.stop_arm()
        gazebo_core.pause_gazebo()
        self._set_initial_model_configuration()
        gazebo_core.unpause_gazebo()
        self.movement_result = self.move_arm_joints(self.init_pos,
                                                    time_from_start=self.action_speed)
        self.move_gripper_joints(self.init_open_gripper,
                                 time_from_start=self.action_speed)
        ...
        # if we don't spwan cube randomly, we can hard code one
            ...
            # Default cube pose on cafe-table top, front-left of arm.
            cube_init_vector = np.array([0.500, -0.150, 0.795], dtype=np.float32)
        ...
        # if we don't have a random pnp goal, we can hard code one
            ...
            # Default pnp goal: lifted off the cafe-table to a place-at-
            # height target above the front-right of the table.
        ...
        # gripper at its open value. PnP episodes start with the gripper
        # open; the agent commands close when contacting the cube. For
        # Robotiq 2F-85 "open" is the LOW end of the knuckle range
        # (gripper_min ≈ 0.0), unlike VX300S where open is gripper_max.
        if self.ee_action_type:
    ...
    def execute_action(self, action):
        ...
        # is queued. Robotiq 2F-85 exposes a single knuckle joint (mimic
        # linkage drives the other finger joints), so we publish a
        # single-element trajectory — no negated pair like the VX300S
        # prismatic-finger setup.
        try:
    ...
    def sample_observation(self):
        ...
        if self.prev_action is None:
            # Bootstrap: assume gripper is at its open value on the first
            # tick. Once the agent's first action lands, self.prev_action
            # is set to the actual 6/4-DOF command in _set_action.
            # Robotiq 2F-85 open = gripper_open_value (low end of knuckle
            # range), not gripper_max as VX300S used.
            if self.ee_action_type:
        ...
        # AND knuckle-close position. Robust lookup by joint name so we
        # don't depend on a fixed joint_pos_all ordering.
        #
        # Robotiq 2F-85 INVERTS the VX300S comparison direction: knuckle
        # position INCREASES as the gripper closes (range 0.0 open ->
        # 0.8 closed), so "closed enough to grasp" means
        # knuckle_pos > grasp_finger_thresh. The default-on-failure value
        # is the open value so a missing joint defaults to "not grasped".
        try:
    ...
    def _get_params(self):
        ...
        # Open / closed reference values. Robotiq 2F-85 closes as the
        # knuckle position grows (open = low, closed = high), opposite
        # of the VX300S prismatic-finger convention. Loaded as separate
        # rosparams so reward / obs code that wants "the open value"
        # doesn't have to know which way gripper_min/max maps.
        ...
        # Grasp-detection thresholds (drive is_grasped in obs + dense
        # reward) and multi-goal lift height. grasp_finger_thresh is
        # interpreted as a LOWER bound on the knuckle for UR5e/Robotiq
        # ("knuckle has closed past X = grasping something").
