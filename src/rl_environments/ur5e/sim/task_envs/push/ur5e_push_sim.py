...
class UR5ePushEnv(ur5e_robot_sim.UR5eRobotEnv):
    """
    ...
    def __init__(self, launch_gazebo: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "UR5ePushEnv" + "_" + ros_port
        else:
            self.node_name = "UR5ePushEnv"
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        # Matches the reach env; UR5e all-zeros puts the arm horizontal
        # through the cafe-table column, so push starts folded with the
        # EE over the workspace.
        self.init_pos = np.array([0.0, -1.5707, 1.5707, -1.5707, -1.5707, 0.0],
                                 dtype=np.float32)

        # Robotiq 2F-85: single-knuckle actuator (mimic linkage drives
        # the other finger joints). Range 0.0 (fully open) to ~0.8 rad
        # (fully closed). Push uses the closed gripper as a flat pusher.
        ...
        # Bypass MoveIt for init: the URDF-zero pose is inside the cafe
        # table column so MoveIt's plan-from-collision fails. Snap joints
        # via SetModelConfiguration (Gazebo paused), then drive both arm
        # and gripper through low-level controllers. Mirrors the reach
        # env's _set_init_params bring-up.
        ...
        self.move_gripper_joints(self.init_close_gripper,
                                 time_from_start=self.action_speed)
        ...
        # if we don't spwan cube randomly, we can hard code one
            ...
            # Default cube pose: on cafe-table top in front-left of the
            # arm (world frame; cafe_table top = z=0.775, cube half-edge
            # adds 0.02).
        ...
        # if we don't have a random push goal, we can hard code one
            ...
            # Default push goal: on the cafe-table top, ~0.2 m left of
            # the cube spawn so the agent has to push laterally.
