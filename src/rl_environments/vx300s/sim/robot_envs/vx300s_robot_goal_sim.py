...
but you need to
    ...
    3. run the env - env = gymnasium.make('VX300SRobotGoalEnv-v0')
"""
register(
    id='VX300SRobotGoalEnv-v0',
    entry_point='rl_environments.vx300s.sim.robot_envs.vx300s_robot_goal_sim:VX300SRobotGoalEnv',
    max_episode_steps=1000,
)


class VX300SRobotGoalEnv(GazeboGoalEnv.GazeboGoalEnv):
    """
    ...
    def __init__(self, ros_port: str = None, gazebo_port: str = None, gazebo_pid=None, seed: int = None,
        ...
        """
        rospy.loginfo("Start Init VX300SRobotGoalEnv Multiros!")
        ...
        #                                 Custom/Interbotix material).
        #                                 Without it Gazebo never starts a
        #                                 controller_manager.
        ...
        # len(q) == kin.num_joints (NOT the full arm DOF). We store
        # (kin, num_joints) so the safety check can slice the action vector
        # correctly per link. Building these once at __init__ amortizes the
        # URDF parse cost.
        self._safety_kin = {}
    ...
    # downstream of gripper_link are covered implicitly by checking
    # gripper_link.
    SAFETY_CHECK_LINKS = (
    ...
    def _check_action_links_safe(self, joint_targets, current_joints=None):
        ...
        count, so we slice ``q[:n]`` before calling ``forward``.

        Rosparams (all under ``/vx300s/``, with sim/real variants):
