...
# Custom robot env
from rl_environments.ur5e.sim.robot_envs import ur5e_robot_goal_sim
...
class UR5ePnPGoalEnv(ur5e_robot_goal_sim.UR5eRobotGoalEnv):
    """
    ...
    def __init__(self, launch_gazebo: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "UR5ePnPGoalEnv" + "_" + ros_port
        else:
            self.node_name = "UR5ePnPGoalEnv"
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        # Matches reach + push + pnp std envs (UR5e all-zeros collides
        # with the cafe-table column).
        self.init_pos = np.array([0.0, -1.5707, 1.5707, -1.5707, -1.5707, 0.0],
                                 dtype=np.float32)

        # Robotiq 2F-85 init poses. PnP resets with the gripper OPEN
        # (gripper_open_value), agent commands close on contact.
        ...
        # Bypass MoveIt for init (URDF zero pose collides with the
        # cafe-table column). Same SetModelConfiguration -> unpause ->
        # raw trajectory pub sequence as the other UR5e task envs.
        # PnP resets with the gripper OPEN.
        ...
        if self.multi_goal:
            cube_xyz = (np.asarray(self.cube_pos, dtype=np.float32)
                        ...
                        else np.array([0.50, -0.15, 0.795], dtype=np.float32))
        ...
        # (gripper_open_value ≈ 0.0), unlike VX300S where open=gripper_max.
        if self.ee_action_type:
    ...
    def sample_observation(self):
        ...
        if self.prev_action is None:
            ...
            # Robotiq 2F-85: open = gripper_open_value (low end), not
            # gripper_max as VX300S used.
            if self.ee_action_type:
        ...
        # knuckle_pos > grasp_finger_thresh.
        try:
    ...
    def _get_params(self):
        ...
        # knuckle grows (open = low, closed = high), opposite of the
        # VX300S prismatic-finger convention.
        ...
        # and multi-goal lift height. grasp_finger_thresh is the LOWER
        # bound on the knuckle for UR5e/Robotiq ("knuckle has closed
        # past X = grasping something").
