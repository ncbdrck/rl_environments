...
register(
    ...
    entry_point='rl_environments.ur5e.real.robot_envs.ur5e_robot_goal_real:UR5eRobotGoalEnv',
    max_episode_steps=1000,
)


class UR5eRobotGoalEnv(RealGoalEnv.RealGoalEnv):
    """
    ...
    def __init__(self, ros_port: str = None, seed: int = None, close_env_prompt: bool = False, action_cycle_time=0.0,
        ...
        """
        rospy.loginfo("Start Init UR5eRobotGoalEnv RealROS")
        ...
        # Real UR5e bring-up — see UR5eRobotEnv (ur5e_robot_real.py) for
        # the wrapper-launch contract: launch ur_robot_driver +
        # Robotiq driver + MoveIt under /ur5e and return only when
        ...
        # MoveIt — SRDF planning groups: "arm" + "gripper".
        ...
        # Robotiq 2F-85: single knuckle (mimic linkage drives other fingers).
        ...
        # FK / IK. UR5e URDF uses bare link names; MoveIt SRDF group
        # "arm" ends at "ee_link".
    ...
    # downstream of ee_link are covered implicitly by checking ee_link.
    SAFETY_CHECK_LINKS = (
        "shoulder_link",
        "upper_arm_link",
        "forearm_link",
    ...
    def _check_action_links_safe(self, joint_targets, current_joints=None):
        """Per-link FK safety + per-joint delta cap.

        See UR5eRobotEnv (ur5e_robot_real.py) for the full rationale on
        the UR5e geometry: arm mounted at base_z on a separate base,
        cafe-table footprint at world (table_center_x, table_center_y),
        link z must clear the base floor everywhere AND the table floor
        when projecting over the table footprint.

        When ``self.enable_strict_safety`` is True (default on the real
        robot env) tighter margins are used.
        """
