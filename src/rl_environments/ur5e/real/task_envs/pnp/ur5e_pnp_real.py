...
# Custom robot env
from rl_environments.ur5e.real.robot_envs import ur5e_robot_real
...
# register(
#     id='UR5ePushSim-v1',
#     entry_point='rl_environments.ur5e.sim.task_envs.push.ur5e_kinect_push_sim_v1:UR5ePushEnv',
#     max_episode_steps=1000,
# )


class UR5ePnPEnv(ur5e_robot_real.UR5eRobotEnv):
    """
    This Task env is for a simple pnp Task with the UR5e robot.
    ...
    def __init__(self, new_roscore: bool = False, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "UR5ePnPEnvReal" + "_" + ros_port
        else:
            self.node_name = "UR5ePnPEnvReal"
        ...
        ros_common.ros_load_yaml(pkg_name="rl_environments", file_name="ur5e_pnp_task_config.yaml", ns="/")
        self._get_params()
        ...
        # extended by ONE scalar gripper command (Robotiq 2F-85 knuckle
        # position; mimic linkage drives the other finger joints). Both
        # EE-action and joint-action modes get the extra DOF. Knuckle
        # range: gripper_min (~0.0, open) to gripper_max (~0.8, closed).
        ...
        # distance + Robotiq knuckle position; kept as float so it can ride in
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        self.init_pos = np.array([0.0, -1.5707, 1.5707, -1.5707, -1.5707, 0.0], dtype=np.float32)

        # close the gripper
        self.init_close_gripper = np.array([0.7], dtype=np.float32)
        # open the gripper (not used)
        self.init_open_gripper = np.array([0.0], dtype=np.float32)
        ...
        # PnP resets with the gripper OPEN (agent commands close on contact).
        self.movement_result = self.move_gripper_joints(self.init_open_gripper)
        ...
        if self.ee_action_type:
            self.prev_action = np.concatenate(
                [self.ee_pos, [self.gripper_open_value]]).astype(np.float32)
        else:
            self.prev_action = np.concatenate(
                [self.init_pos, [self.gripper_open_value]]).astype(np.float32)
        ...
        if self.init_done:
            ...
            if _js_last is None or (_js_now - _js_last) > _js_threshold:
                rospy.logwarn_throttle(
                    ...
                    "skipping env_loop tick — check the UR5e driver "
                    "(ur_robot_driver) + network cable."
                )
                try:
                    self.move_UR5E_object.stop_arm()
                except Exception:
    ...
    def execute_action(self, action):
        ...
        The gripper scalar is ALWAYS treated as an absolute Robotiq 2F-85
        knuckle position in [gripper_min, gripper_max] regardless of
        ``delta_action`` (open/close is closer to a discrete decision than
        a small delta; matches FetchPickAndPlace's gripper convention).
        Knuckle range: gripper_min ≈ 0.0 (fully open) to gripper_max
        ≈ 0.8 (fully closed). A single value is published; the other
        finger joints follow via the URDF mimic linkage.
        ...
        if self.ee_action_type:
            ...
            if self.workspace_space.contains(action):
                ...
                if IK_found:
                    # Per-link FK safety (UR5eRobotEnv._check_action_links_safe).
        ...
        # is queued. Robotiq 2F-85: single knuckle joint (mimic linkage
        # drives the other finger joints), so publish a single-element
        # trajectory — no negated pair like the VX300S prismatic setup.
        try:
            self.move_gripper_joints(
                q_positions=[gripper_cmd],
                time_from_start=self.action_speed,
            )
    ...
    def sample_observation(self):
        ...
        # /cube_pose topic (vision pipeline lives outside this env).
            ...
            if self.ee_action_type:
                ...
                prev_action = np.concatenate(
                    [np.asarray(self.joint_values, dtype=np.float32),
                     [self.gripper_open_value]]).astype(np.float32)
        else:
        ...
        try:
            _kn_idx = self.joint_state.name.index("robotiq_85_left_knuckle_joint")
            _knuckle_pos = float(self.joint_state.position[_kn_idx])
        except (ValueError, AttributeError, IndexError):
            _knuckle_pos = self.gripper_open_value  # default: open
        _dist_ee_cube = float(np.linalg.norm(cube_rel_to_ee))
        self.is_grasped = float(
            (_dist_ee_cube < self.grasp_dist_thresh)
            and (_knuckle_pos > self.grasp_finger_thresh)
        )
        ...
        # Since we only look for Sparse or Dense, we don't need to check if it's Dense
            ...
            # for normal dense reward
                ...
                if done:
                    ...
                    # Pre-grasp: weight EE→cube distance (encourage approach).
                    # Post-grasp: weight cube→goal distance + a grasp bonus.
                    # Without staging the agent gets the same cube→goal
                    # signal regardless of whether it's actually holding
                    # the cube, which slows credit assignment.
    ...
    def _get_params(self):
        ...
        """
        # Action Space
        self.min_joint_values = rospy.get_param('/ur5e/min_joint_pos')
        self.max_joint_values = rospy.get_param('/ur5e/max_joint_pos')

        # Gripper scalar bounds for the action space — Robotiq 2F-85
        # knuckle position in radians (single joint; mimic linkage
        # drives the other finger joints).
        self.gripper_min = float(rospy.get_param('/ur5e/gripper_min'))
        self.gripper_max = float(rospy.get_param('/ur5e/gripper_max'))

        # Open / closed reference values. Robotiq closes as knuckle
        # grows (open = low, closed = high), opposite of VX300S.
        self.gripper_open_value = float(rospy.get_param('/ur5e/gripper_open_value'))
        self.gripper_closed_value = float(rospy.get_param('/ur5e/gripper_closed_value'))

        # Grasp-detection thresholds (drive is_grasped + dense reward)
        # and multi-goal lift height. grasp_finger_thresh is interpreted
        # as a LOWER bound on the knuckle for UR5e/Robotiq.
        self.grasp_dist_thresh = float(rospy.get_param('/ur5e/grasp_dist_thresh'))
        self.grasp_finger_thresh = float(rospy.get_param('/ur5e/grasp_finger_thresh'))
        self.lift_height = float(rospy.get_param('/ur5e/lift_height'))

        # Observation Space
        self.position_ee_max = rospy.get_param('/ur5e/position_ee_max')
        self.position_ee_min = rospy.get_param('/ur5e/position_ee_min')
        self.rpy_ee_max = rospy.get_param('/ur5e/rpy_ee_max')
        self.rpy_ee_min = rospy.get_param('/ur5e/rpy_ee_min')
        self.linear_distance_max = rospy.get_param('/ur5e/linear_distance_max')
        self.linear_distance_min = rospy.get_param('/ur5e/linear_distance_min')
        self.max_distance = rospy.get_param('/ur5e/max_distance')
        self.min_joint_vel = rospy.get_param('/ur5e/min_joint_vel')
        self.max_joint_vel = rospy.get_param('/ur5e/max_joint_vel')
        self.min_joint_angles = rospy.get_param('/ur5e/min_joint_angles')
        self.max_joint_angles = rospy.get_param('/ur5e/max_joint_angles')
        self.position_cube_min = rospy.get_param('/ur5e/position_cube_min')
        self.position_cube_max = rospy.get_param('/ur5e/position_cube_max')
        self.rpy_cube_min = rospy.get_param('/ur5e/rpy_cube_min')
        self.rpy_cube_max = rospy.get_param('/ur5e/rpy_cube_max')
        ...
        self.linear_velocity_cube_min = rospy.get_param('/ur5e/linear_velocity_cube_min')
        self.linear_velocity_cube_max = rospy.get_param('/ur5e/linear_velocity_cube_max')
        self.angular_velocity_cube_min = rospy.get_param('/ur5e/angular_velocity_cube_min')
        self.angular_velocity_cube_max = rospy.get_param('/ur5e/angular_velocity_cube_max')
        self.cube_rel_to_ee_min = rospy.get_param('/ur5e/cube_rel_to_ee_min')
        self.cube_rel_to_ee_max = rospy.get_param('/ur5e/cube_rel_to_ee_max')

        # Goal space
        self.position_goal_max = rospy.get_param('/ur5e/position_goal_max')
        self.position_goal_min = rospy.get_param('/ur5e/position_goal_min')

        # Achieved goal
        self.position_achieved_goal_max = rospy.get_param('/ur5e/position_achieved_goal_max')
        self.position_achieved_goal_min = rospy.get_param('/ur5e/position_achieved_goal_min')

        # Desired goal
        self.position_desired_goal_max = rospy.get_param('/ur5e/position_desired_goal_max')
        self.position_desired_goal_min = rospy.get_param('/ur5e/position_desired_goal_min')

        # Tolerances
        self.reach_tolerance = rospy.get_param('/ur5e/reach_tolerance')
        ...
        self.joint_state_timeout_s = float(
            rospy.get_param('/ur5e/joint_state_timeout_s', 0.5)
        )

        # Variables related to rewards
        self.step_reward = rospy.get_param('/ur5e/step_reward')
        self.mult_dist_reward = rospy.get_param('/ur5e/multiplier_dist_reward')
        self.reached_goal_reward = rospy.get_param('/ur5e/reached_goal_reward')
        self.joint_limits_reward = rospy.get_param('/ur5e/joint_limits_reward')
        self.none_exe_reward = rospy.get_param('/ur5e/none_exe_reward')
        self.not_within_goal_space_reward = rospy.get_param('/ur5e/not_within_goal_space_reward')

        # workspace
        self.workspace_max = rospy.get_param('/ur5e/workspace_max')
        self.workspace_min = rospy.get_param('/ur5e/workspace_min')
    ...
    def _launch_roscore(self, port=None, default_port=False, set_new_master_vars=False):
        """Launch a new roscore.

        Mirrors realros.utils.ros_common.launch_roscore via the framework's
        standard launcher — the real-side variant accepts default_port for
        11311 attachment.
        """
        ros_port = ros_common.launch_roscore(port=port, default_port=default_port,
                                             set_new_master_vars=set_new_master_vars)
        return ros_port
