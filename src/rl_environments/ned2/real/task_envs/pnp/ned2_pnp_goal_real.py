...
# from multiros.utils import gazebo_core  # not used in real env
# from multiros.utils import gazebo_models  # not used in real env
# from multiros.utils import gazebo_physics  # not used in real env
...
"""
NED2 Pick-and-Place Goal Task Environment (real).

Ported from the RX200 PnP goal real env (rx200_pnp_goal_real.py) with the
NED2-specific conventions taken from ned2_pnp_real.py (the std real sibling,
authoritative for every NED2-real-PnP decision) and ned2_pnp_goal_sim.py
(authoritative for every goal-env convention):
  * 6 arm joints (vs 5 on the RX200) → action dim 6+1=7 (joint mode) or
    3+1=4 (EE mode). The trailing "+1" is the gripper scalar.
  * NED2RobotGoalEnv (real) parent class. Real-side keeps ``use_kinect`` /
  ...
  * SAFETY_CHECK_LINKS and ``_check_action_links_safe`` live on the
    ...
    EE and joint branches (parity with ned2_pnp_real / ned2_push_goal_real).
  ...
  * Cube tracking is via the external ``/cube_pose`` topic just like
    ned2_pnp_real, with the same auto_launch_cube_tracker convenience
    that wires rl_envs_cube_tracker if the user opts in.

PnP-specific (mirror ned2_pnp_real.py — the std real sibling):
  * Gripper API: NED2's tools-commander action server only accepts
    ...
    RX200 PnP goal for cross-robot policies), and execute_action
    discretizes at ``gripper_cmd_mid`` just before dispatch.
    State-change-only caching via ``last_gripper_state`` avoids spamming
    the blocking action server every env-loop tick.
...
Goal-env-specific (mirror ned2_pnp_goal_sim.py):
  * GoalEnv API: compute_reward / compute_terminated / compute_truncated
    (no underscore) — HER plumbing lives in compute_reward; HER-safety
    branch returns -dist2goal when ``is_her`` so step side-effects don't
    bleed into relabeled samples.
  * _get_achieved_goal returns cube_pos (3-D); _get_desired_goal flips
    between ``intermediate_goal`` and ``pnp_goal`` under multi_goal=True.
    achieved_goal stays 3-D — is_grasped rides in the observation slot,
    NOT achieved_goal, so HER's cube-position relabeling stays
    well-defined.
  * cube_pos / cube_ori cache seeded in _set_init_params from
    cube_init_vector so the first reset()'s _get_achieved_goal returns
    a real value rather than zeros (mirrors the push goal pattern).

Key sim/real divergences for NED2 PnP-Goal specifically:
  ...
  * Sim has Gazebo's guaranteed /clock so it doesn't need the joint-
    ...
    ned2_reach_real / ned2_push_real / ned2_pnp_real introduced so we
    stop the arm if the niryo driver / cable drops mid-tick.
  ...
  * NED2RobotGoalEnv (real) has NO ``gripper`` kwarg — the URDF + tools
...
Std vs Goal divergences carried over from RX200:
  * Goal env builds a Dict observation_space with
    ``observation``/``achieved_goal``/``desired_goal`` slots — the std
    env exposes a flat (or sensor-Dict) observation_space.
  * sample_observation returns only the ``observation`` slot; the outer
    Dict is assembled by GazeboGoalEnv.step via _get_achieved_goal /
    _get_desired_goal hooks.
  * Reward / termination / truncation are exposed via the
    compute_reward / compute_terminated / compute_truncated (no
    underscore) GoalEnv hooks, with the HER-safety branch in
    compute_reward returning -dist2goal so step side-effects don't
    survive HER relabelling.
"""


class NED2PnPGoalEnv(ned2_robot_goal_real.NED2RobotGoalEnv):
    """
    This Task env is for a simple pnp Task with the NED2 robot. (Goal-conditioned)
    ...
    Here
        ...
        * Observation - Dict ({observation, achieved_goal, desired_goal}) where the
          ``observation`` slot is either the flat traditional vector, an RGB image,
          or a nested sensor Dict for combined modalities.
    ...
    Init Args:
        ...
        * use_kinect: Whether to use the kinect sensor for cv_image_rgb/depth.
        * use_zed2: Whether to use the ZED2 sensor.
        * cube_pose_topic: Topic to subscribe to for cube pose (geometry_msgs/PoseStamped).
            Falls back to config-default cube_init_pos when no recent message arrives.
        * auto_launch_cube_tracker: If True, launch rl_envs_cube_tracker/<camera>.launch
            via the managed-process registry. Default False — vision is deliberately
            external so mocap / YOLO / custom-detector users can plug in their own pipeline.
        * cube_tracker_camera: "kinect2" or "zed2" — picks the launch file to bring up
            when auto_launch_cube_tracker is True.
        * cube_tracker_target_frame: Optional TF frame to which the tracker should
            project the cube pose (passed as the ``target_frame`` arg to the launch file).
        * multi_goal: Whether to use the multi-goal curriculum (intermediate lift goal
            then final pnp goal).
    ...
    def __init__(self, new_roscore: bool = False, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "NED2PnPGoalEnvReal" + "_" + ros_port
        else:
            self.node_name = "NED2PnPGoalEnvReal"
        ...
        Init super class.
        """
        # NED2RobotGoalEnv (real) keeps ``use_kinect`` / ``use_zed2`` kwargs —
        # only the sim parent renamed to ``use_camera``. NOTE: unlike the
        # sim goal env, the real parent has NO ``gripper`` kwarg — the URDF /
        # controllers / tools_commander are brought up externally by
        ...
        # subscribe to ``cube_pose_topic`` (geometry_msgs/PoseStamped) and
        # consult the freshest sample inside sample_observation. If no
        ...
        # every env loop tick (the action server blocks on
        # wait_for_result, so spamming would dominate loop time).
        ...
        # Seeded in _set_init_params with the cube_init_vector; mirrors
        # the push goal port's first-reset HER safety pattern.
        self.cube_pos = None
        self.cube_ori = None
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        Here we
            ...
            3. Prompt the user to physically place the cube (real robot;
               there's no Gazebo spawn/remove on hardware)
            4. Find random goal position to pnp the cube to if random_goal is True
        ...
        # later in this method, after the cube has been (re)placed so
        # cube_init_pos reflects the user's intended starting pose.
        # multi_goal=False leaves intermediate_goal=None and
        # intermediate_reached untouched.
        self.is_grasped = 0.0
        self.intermediate_goal = None
        self.intermediate_reached = False
        ...
        # Open the gripper at episode start so the agent always begins
        # with an empty hand. NED2's gripper API is the tools-commander
        # action server — string commands only. Seed last_gripper_state
        # so the first agent-requested "open" inside execute_action is
        # NOT dropped as redundant (cache miss vs None forces dispatch);
        # the first agent-requested "close" will dispatch as expected.
        try:
        ...
        # --------------- Cube placement (real robot)
        # On real hardware the cube is physically placed by the user; there
        # is no spawn/remove machinery. The external vision pipeline is
        # expected to publish poses on self.cube_pose_topic (see
        # _read_latest_cube_pose). Use cube_init_pos from config purely
        # for the initial marker / fallback.
        rospy.loginfo(
            "Episode reset on real robot: please physically place the cube at "
            "the expected start position before continuing. "
            "Cube pose will be tracked via the cube_pose topic during the episode."
        )
        cube_init_vector = np.asarray(self.cube_init_pos, dtype=np.float32)
        ...
        # Seed the cube pose cache with the user-intended init pose so
        # _get_achieved_goal has something real to return before the first
        # sample_observation tick — otherwise reset()'s obs dict would
        # carry achieved_goal=zeros, which silently corrupts HER's first
        # relabeled sample. Mirrors the push goal port's first-reset HER
        # safety pattern. Once the external /cube_pose topic publishes,
        # sample_observation refreshes self.cube_pos with the live value.
        ...
        # current cube pose. On hardware the cube is physically placed,
        # so we seed from cube_init_pos (the fallback the marker was
        # also placed at); once a real /cube_pose message arrives, the
        # agent's distance signal will track the actual cube pose, but
        # the lift target stays anchored to the user's intended drop
        # point.
        if self.multi_goal:
            cube_xyz = (np.asarray(self.cube_pos, dtype=np.float32)
                        if getattr(self, "cube_pos", None) is not None
                        else cube_init_vector.copy())
    ...
    def execute_action(self, action):
        ...
        state we dispatched (``last_gripper_state``). The action server
        blocks on wait_for_result, so spamming would dominate loop time
        on hardware just as it would in sim.
        ...
        if self.ee_action_type:
            ...
            if self.workspace_space.contains(action):
                ...
                if IK_found:
                    # Per-link FK safety (NED2RobotGoalEnv._check_action_links_safe).
                    ...
                    # the full rationale. On hardware this is the last guard
                    # before move_arm_joints publishes to the niryo driver.
        ...
        # would dominate the loop time. ``last_gripper_state`` is seeded
        # to "open" in _set_init_params after the episode-start open, so
        # the first agent-requested "close" forces a dispatch.
    ...
    def sample_observation(self):
        ...
        # --- Get the current cube position and orientation via the external
        # /cube_pose topic (vision pipeline lives outside this env). Sim
        # reads from Gazebo via get_model_pose("red_cube"); on real we
        # subscribe and fall back to cube_init_pos when stale.
        _cube_read = self._read_latest_cube_pose()
        if _cube_read is None:
            if self.log_internal_state:
                rospy.logwarn_throttle(
                    5.0,
                    f"No cube pose on {self.cube_pose_topic} (or stale); "
                    "falling back to config cube_init_pos. Wire up a vision "
                    "pipeline that publishes geometry_msgs/PoseStamped on "
                    "this topic for actual cube tracking.",
                )
            self.cube_pos = np.asarray(self.cube_init_pos, dtype=np.float32)
            self.cube_ori = np.zeros(3, dtype=np.float32)
            cube_pose_done = False
        else:
            self.cube_pos, self.cube_ori = _cube_read
            cube_pose_done = True
        ...
        # YAML is the position (m) below which we treat the fingers as
        # "closed on something". On hardware the niryo driver publishes
        # the mors joint state from the gripper's encoder; calibrate
        # the threshold against the actual closed-on-cube reading
        # (which may differ slightly from the URDF stroke limit).
        try:
    ...
    def _get_params(self):
        ...
        # tuned for NED2's tighter range AND calibrated on hardware
        # against the niryo driver's encoder reading on a real cube.
        ...
        # External cube-pose pipeline: timeout for treating a cached pose
        # as stale (sec) and a fallback init pos for when no message has
        # ever arrived. Both default to sensible values so the env can
    ...
    def _cube_pose_callback(self, msg):
        """Subscriber callback for the external cube-pose topic.

        Stores the latest message + receipt time so sample_observation
        can decide whether the pose is fresh enough to trust.
        """
        self._latest_cube_pose_msg = msg
        self._latest_cube_pose_time = rospy.get_time()

    def _read_latest_cube_pose(self):
        """Return (cube_pos_array, cube_ori_rpy_array) or None if stale.

        "Stale" = no message received within self.cube_pose_timeout_s
        seconds. Callers that get None should fall back to
        self.cube_init_pos and log a warning. The fallback path lets the
        env construct and run even when no vision pipeline is wired up —
        useful for code review or dry-run experiments. Production
        deployments should monitor info['cube_pose_stale'] (set by
        sample_observation when this helper returns None).
        """
        if self._latest_cube_pose_msg is None or self._latest_cube_pose_time is None:
            return None
        age = rospy.get_time() - self._latest_cube_pose_time
        if age > self.cube_pose_timeout_s:
            return None
        pose = self._latest_cube_pose_msg.pose
        cube_pos = np.array([pose.position.x, pose.position.y, pose.position.z], dtype=np.float32)
        cube_ori = np.array(
    ...
    @staticmethod
    def _launch_roscore(port=None, default_port=False, set_new_master_vars=False):
        """Launch a new roscore.

        Mirrors ned2_pnp_real's static helper — the real-side variant
        accepts default_port for 11311 attachment and changes the active
        ROS master to the freshly launched roscore.
        """
        if port is None:
