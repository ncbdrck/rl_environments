...
# Custom robot env
from rl_environments.ur5e.real.robot_envs import ur5e_robot_goal_real
...
# register(
#     id='UR5ePushSim-v1',
#     entry_point='rl_environments.ur5e.sim.task_envs.push.ur5e_kinect_push_sim_v1:UR5ePushGoalEnv',
#     max_episode_steps=1000,
# )


class UR5ePnPGoalEnv(ur5e_robot_goal_real.UR5eRobotGoalEnv):
    """
    ...
    def __init__(self, new_roscore: bool = False, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "UR5ePnPGoalEnvReal" + "_" + ros_port
        else:
            self.node_name = "UR5ePnPGoalEnvReal"
        ...
        if self.init_done:
            ...
            if _js_last is None or (_js_now - _js_last) > _js_threshold:
                rospy.logwarn_throttle(
                    ...
                    "skipping env_loop tick — check the UR5e "
                    "driver + USB cable."
                )
                try:
                    self.move_UR5E_object.stop_arm()
    ...
    def execute_action(self, action):
        ...
        # drives other fingers), so publish a single-element trajectory.
        try:
    ...
    def _get_params(self):
        ...
        # Gripper scalar bounds — Robotiq 2F-85 knuckle action range.
        self.gripper_min = float(rospy.get_param('/ur5e/gripper_min'))
        self.gripper_max = float(rospy.get_param('/ur5e/gripper_max'))

        # Open / closed reference values (Robotiq closes as knuckle grows).
        ...
        # Grasp-detection thresholds + multi-goal lift height.
        # grasp_finger_thresh: LOWER bound on knuckle ("closed enough").
