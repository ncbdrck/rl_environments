...
# register(
#     id='UR5eReacherSim-v2',
#     entry_point='rl_environments.ur5e.sim.task_envs.reach.ur5e_reach_sim_v2:UR5eReacherEnv',
#     max_episode_steps=1000,
# )


class UR5eReacherEnv(ur5e_robot_sim.UR5eRobotEnv):
    """
    ...
    def __init__(self, launch_gazebo: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "UR5eReacherEnvSim" + "_" + ros_port
        else:
            self.node_name = "UR5eReacherEnvSim"
        ...
        if environment_loop_rate is not None:
            ...
            # Normal mode reuses the same obs_r/reward_r/... cache
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        # Initial robot pose. UR5e all-zeros is arm-horizontal which
        # collides with the cafe-table column; this folds the arm
        # upright with the EE over the workspace area instead.
    ...
    def _set_action(self, action):
        ...
        is what actually calls ``execute_action`` (with action repeats);
        ``GazeboBaseEnv.step`` then sleeps ``action_cycle_time``
