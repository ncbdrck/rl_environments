...
class UR5ePushGoalEnv(ur5e_robot_goal_sim.UR5eRobotGoalEnv):
    """
    ...
    def __init__(self, launch_gazebo: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "UR5ePushGoalEnv" + "_" + ros_port
        else:
            self.node_name = "UR5ePushGoalEnv"
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        ...
        # Matches reach + push std envs (UR5e all-zeros collides with the
        # cafe-table column, so we start folded with the EE over the
        # workspace).
        self.init_pos = np.array([0.0, -1.5707, 1.5707, -1.5707, -1.5707, 0.0],
                                 dtype=np.float32)

        # Robotiq 2F-85: single-knuckle actuator. Range 0.0 (open) to
        # ~0.8 rad (closed). Push uses the closed gripper as a paddle.
        ...
        # Bypass MoveIt for init (URDF-zero collides with table). Snap
        # joints via SetModelConfiguration, then drive arm + gripper via
        # the low-level trajectory controllers. Same sequence as the
        # reach / push std envs.
        ...
        # if we don't have a random push goal, we can hard code one
            ...
            # Default push goal: on the cafe-table, ~0.2 m left of cube.
